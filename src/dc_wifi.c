...
void dc_wifi_event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
...
esp_err_t dc_wifi_init_sta(void)
{
  ...
  ESP_ERROR_CHECK(esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &dc_wifi_event_handler, NULL, &instance_any_id));
  ESP_ERROR_CHECK(esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &dc_wifi_event_handler, NULL, &instance_got_ip));
